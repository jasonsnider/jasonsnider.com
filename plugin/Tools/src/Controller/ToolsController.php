...
{   
    ...
    public function php_hashes()
    {
        ...
        if (isset($_POST['string_to_hash'])) {
            $string = $_POST['string_to_hash'];
        }
    ...
    public function string_analyzer()
    {
        if (!empty($_POST)) {
            $this->View->vars = array(
                'character_count' => strlen($_POST['string']),
                'is_numeric' => is_numeric($_POST['string']) ? 'Yes' : 'No'
            );
        }
    }
